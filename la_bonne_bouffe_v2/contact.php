...
if (!empty($_POST)) {
	...
	*/
	...
	if(!empty($post['FirstName-take'])){ //Si le champ prénom n'est pas vide alors on vérifie qu'il commence bien en majuscule
		...
		if(!((bool)preg_match('#^[A-Z]+[a-zA-ZáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ]#', $post['FirstName-take']))){ 
	...
	if(!empty($post['LastName-take'])){ //Si le champ nom n'est pas vide alors on vérifie qu'il commence bien en majuscule
		...
		if(!((bool)preg_match('#^[A-Z]+[a-zA-ZáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ]#', $post['LastName-take']))){ 
